...
static var test_cases: Array[TestCase] = [
	TestCase.new(-1, -1, "BoardPosition(row=-1, col=-1)", -4, true, false),
	TestCase.new(0, 0, "BoardPosition(row=0, col=0)", 0, false, false),
	TestCase.new(0, 1, "BoardPosition(row=0, col=1)", 1, false, false),
	TestCase.new(0, 2, "BoardPosition(row=0, col=2)", 2, false, false),
	TestCase.new(0, 3, "BoardPosition(row=-1, col=-1)", -4, true, true),
	TestCase.new(1, 2, "BoardPosition(row=1, col=2)", 5, false, false),
	TestCase.new(3, 1, "BoardPosition(row=-1, col=-1)", -4, true, true),
]
...
func test_properties(test_case: TestCase = use_parameters(test_cases)) -> void:
	...
	assert_eq(actual.is_invalid(), test_case.is_invalid)
